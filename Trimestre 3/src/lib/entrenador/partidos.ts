...
export async function agregarEvento(formData: FormData): Promise<void> {
  ...
  if (jugadorId) {
    ...
    if (jugador) {
      ...
      if (tipo === "amarilla") updates.tarjetas_amarillas = (jugador.tarjetas_amarillas || 0) + 1;
      if (tipo === "roja") updates.tarjetas_rojas = (jugador.tarjetas_rojas || 0) + 1;
  ...
  if (evento) {
    ...
    if (evento.jugador_id) {
      ...
      if (jugador) {
        ...
        if (evento.tipo === "amarilla") updates.tarjetas_amarillas = Math.max(0, (jugador.tarjetas_amarillas || 0) - 1);
        if (evento.tipo === "roja") updates.tarjetas_rojas = Math.max(0, (jugador.tarjetas_rojas || 0) - 1);
